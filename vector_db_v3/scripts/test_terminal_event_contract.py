...
def assert_compliance_fields(events: list[dict], expect_status: str) -> None:
    ...
    for key in required:
        if key not in terminal:
            raise AssertionError(f"missing compliance field: {key}")
    if terminal.get("compliance_status") != expect_status:
...
def main() -> int:
    ...
    code, out, err = run([str(cli), "build-top-clusters", "--path", str(data_dir)], root, env=env_pass)
    if code != 0:
    ...
    code, out, err = run([str(cli), "build-mid-layer-clusters", "--path", str(data_dir)], root, env=env_pass)
    if code != 0:
    ...
    code, out, err = run([str(cli), "build-lower-layer-clusters", "--path", str(data_dir)], root, env=env_pass)
    if code != 0:
    ...
    code, out, err = run([str(cli), "build-final-layer-clusters", "--path", str(data_dir)], root, env=env_pass)
    if code != 0:
    ...
    for stage_cmd, stage_id in [
        ("build-top-clusters", "top"),
        ("build-mid-layer-clusters", "mid"),
        ("build-lower-layer-clusters", "lower"),
        ("build-final-layer-clusters", "final"),
    ]:
        env_compliance = dict(os.environ)
        env_compliance["VECTOR_DB_V3_FORCE_COMPLIANCE_FAIL"] = "1"
        code, out, err = run([str(cli), stage_cmd, "--path", str(data_dir)], root, env=env_compliance)
        if code != 1:
            return fail(f"forced compliance failure should return exit code 1 for {stage_cmd}", out, err)
        events, _ = parse_lines(out)
        try:
            stage_fail = next(e for e in events if e["event_type"] == "stage_fail")
