...
| Streamed binary ingest path for `bulk-insert-bin` (no full materialization) with malformed-input deterministic handling | `CLI_CONTRACT.md`, `TEST_GATE_CONTRACT.md` | G1, G3, G4 |
| Card 9 validation runner evidence (`run_card9_validation.py`) for streaming functional/negative/perf matrix | `TEST_GATE_CONTRACT.md`, `TRACEABILITY_MATRIX.md` | G1, G3, G4, G7 |
| Metadata-overhead optimization parity for write-time byte/checksum accounting (no post-write readback/stat dependency in hot paths) | `ARTIFACT_CONTRACT.md`, `TERMINAL_EVENT_CONTRACT.md`, `TEST_GATE_CONTRACT.md` | G1, G3, G7 |
| Card 10 validation runner evidence (`run_card10_validation.py`) for metadata parity and no-regression perf probe | `TEST_GATE_CONTRACT.md`, `TRACEABILITY_MATRIX.md` | G1, G3, G4, G7 |
