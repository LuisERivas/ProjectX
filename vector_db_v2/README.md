...
- D-007: Clustering hierarchy is Top -> Mid -> Lower -> Final, with Final layer running passthrough finalization only for eligible gate-fail (`stop`) Lower-layer centroid datasets; gate `continue` branches remain in Lower processing (no cross-centroid mixing) (see `02`, `05`, `06`)
- D-011: Final-layer per-cluster artifact contract in M1 is minimally normative: `manifest.json`, `assignments.json`, `cluster_summary.json` under `final_layer_clustering/final_cluster_<id>/`, plus aggregate `final_layer_clustering/FINAL_LAYER_CLUSTERS.json` (see `03`, `05`, `06`, `07`)
- D-012: Final-layer assignments contract in M1 is canonical (array of `{embedding_id, final_cluster_id}` rows sorted ascending by `embedding_id`; unique IDs; cardinality equals processed embeddings) with machine-readable per-cluster output status reporting (see `05`, `06`, `07`, `GLOSSARY`)
