...
| 05/06-FINAL-ELIG | Final eligibility: gate `stop` only (gate semantics preserved) | `write_final_artifacts()` | Implemented | pipeline + final artifact checks | `final_cluster_id`, `source_lower_centroid_id` | Empty-leaf suppression policy can be expanded |
| 05/06/07-ASSIGNMENTS | Canonical `assignments.json` schema + ordering + uniqueness | final assignments writer | Implemented | `test_m1.cpp` + schema checks | `assignments_file_present` | Runtime schema validator can be strengthened |
